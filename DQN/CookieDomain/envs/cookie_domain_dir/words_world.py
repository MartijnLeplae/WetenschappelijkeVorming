...
class WordsWorld(gym.Env):
    ...
    def step(self, letter):
        ...
        if len(self.state)>self.repr_length and self._word(self.state[-(self.repr_length+1):]) in self._word(self.nb_goal):
            reward = 1
        else:
    ...
    # and '0' is not a memeber of self.actions.
    def _word(self, lst):
        return ''.join([self.actions[i-1] for i in lst])

    # This is the function where you decide wath the agent (=neural network) can 'see'.
